...
function checkTableName($shortTName )
{
	...
	if ("emp_deduciones" == $shortTName )
		return true;
	if ("emp_tipoemp" == $shortTName )
		return true;
	if ("emp_mstemp" == $shortTName )
		return true;
	return false;
}
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("emp_deduciones");
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("emp_tipoemp");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("emp_mstemp");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
...
function GetUserPermissionsStatic( $table )
{
	...
	if( $table=="emp_deduciones" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
	}
	if( $table=="emp_tipoemp" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
...
function GetBaseScriptsForPage($isDisplayLoading, $additionalScripts = "", $customText = "")
{
	...
	$result = "";
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/loadfirst.js?35571")."\"></script>";


	$result .= $additionalScripts;
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/lang/".getLangFileName(mlang_getcurrentlang()).".js?35571")."\"></script>";
 ...
 */
function getListOfSuggests( $sfields, $table, $numberOfSuggests, $searchOpt, $searchFor, $searchField = '', $detailKeys = array() )
{
	...
	foreach( $sfields as $f )
	{
		...
		$dc->filter = DataCondition::_And( array( 
			$fieldControl->getSearchCondition( $searchFor, $searchOpt ),
...
function setProjectCookie( $name, $value, $expires = 0, $httpOnly = false, $sameSiteStrict = false) {
	runner_setcookie( $name, $value, $expires, projectPath(), "", false, $httpOnly, $sameSiteStrict );
}
